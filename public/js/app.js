  ...
  return `
    <div class="page-header" style="display:flex;align-items:center;
      ...
      <div>
        ...
        <div class="page-sub">Model Context Protocol — how Claude CLI reads Fulcrum data</div>
      </div>
...
function bindBDCQ() {
  ...
  // Auto-save on client/scope field changes
  setTimeout(() => {
  ...
  window.bdcqLoadEditor = async () => {
    ...
    const countries       = [...document.querySelectorAll(".bdcq-country-check")].filter(cb => cb.checked).map(cb => cb.value);
    ...
    try {
      ...
      // Add AI + Answer + Notes fields to each row
  ...
  const AI_COLS   = ["SAP Standard Value", "Watch Out / Constraint", "Answer", "Notes"];
  ...
  function renderEditorCards() {
    ...
    listEl.innerHTML = groups.map(g => {
      ...
      const questionCards = groupRows.map((row) => {
        ...
        const isLocaleRow = /^[A-Za-z]{2}$/.test(row.Section || "");
        const localeBadge = isLocaleRow
          ? `<span style="font-size:9px;font-weight:700;padding:2px 6px;background:#dbeafe;color:#1d4ed8;border-radius:3px;flex-shrink:0;margin-top:2px">🌍 ${escHtml(row.Section)}</span>`
          : "";

        return `
        <div style="padding:14px 0;border-bottom:1px solid var(--border)${isLocaleRow ? ";border-left:3px solid #93c5fd;padding-left:10px" : ""}">
  ...
  window.bdcqEnrich = async () => {
    ...
    // Group rows by domain to keep Claude context focused
    const domainGroups = {};
    ...
    for (const [domain, entries] of Object.entries(domainGroups)) {
      // Process domain in batches of BATCH
      for (let b = 0; b < entries.length; b += BATCH) {
        const batch = entries.slice(b, b + BATCH);
        if (progressEl) progressEl.innerHTML =
          `✨ Enriching <strong>${domain}</strong> — batch ${Math.floor(b / BATCH) + 1} / ${Math.ceil(entries.length / BATCH)} &nbsp;·&nbsp; ${done}/${total} rows done…`;
        ...
        try {
          ...
          if (r.ok && Array.isArray(r.enrichments)) {
            ...
            const errMsg = r.error || "Unknown error from server";
            if (progressEl) progressEl.innerHTML =
              `❌ Enrichment failed for <strong>${domain}</strong>: ${escHtml(errMsg)}
               <br><span style="font-size:11px;color:var(--text-muted)">
               Check that Claude CLI is installed and START.bat added it to PATH.
               Run <code>claude --version</code> in a terminal to verify.</span>`;
    ...
    if (progressEl) {
      progressEl.innerHTML = `✅ Enrichment complete — ${total} rows enriched. Review AI values before exporting.`;
      setTimeout(() => { if (progressEl) progressEl.style.display = "none"; }, 5000);
    }
    if (btnEl) { btnEl.disabled = false; btnEl.textContent = "✨ Re-Enrich"; }

    // Send notification directly here — total is the exact count from this run only
    if (total > 0) {
      try {
      ...
      } catch { /* notify server not running — silent */ }
    }
  };
  ...
  window.bdcqExportExcel = async () => {
    ...
    try {
      ...
      const r = await api.post("/api/bdcq/export-editor", { filename, rows, columns, clientName, overviewText, clientLogo, partnerLogo });
      if (r.ok) {
