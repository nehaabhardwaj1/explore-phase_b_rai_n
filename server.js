...
function resolveClaudeExe() {
  ...
  return "claude"; // last resort — let OS PATH try
}
...
const _claudeIsCmd = _isWinServer && _claudeExe.toLowerCase().endsWith(".cmd");
...
JSON only. No markdown.`;
    ...
    try {
      const scored = await new Promise((resolve, reject) => {
        let timedOut = false;
        const proc  = spawn(_claudeExe, _claudeArgs(CLAUDE_MODEL), _claudeOpts());
...
app.post("/api/kdd/generate", async (req, res) => {
  ...
  exec(cmd, { cwd: ROOT }, (err, stdout) => {
    ...
    if (err) {
      ...
      // Notify — send explicit detail so email shows real scope items, not "—"
      if (filename) {
        ...
        const nr = require("http").request(
          ...
          () => {}  // response callback — ignore response body
        );
        nr.on("error", () => {}); // notify server not running — silent
